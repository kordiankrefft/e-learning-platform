...
export default function LessonAttachmentsPage() {
  ...
  const [items, setItems] = useState<LessonAttachmentDto[]>([]);
    ...
    (async () => {
      try {
        ...
        if (lessonIdFilter != null) {
          const res = await lessonAttachmentsApi.getForLesson(lessonIdFilter);
          setItems(res.data ?? []);
          return;
        }
        ...
        const attRes = await Promise.allSettled(
          lessons.map((l) => lessonAttachmentsApi.getForLesson(l.id))
        );

        const merged: LessonAttachmentDto[] = [];
        ...
        uniq.sort((a, b) => a.lessonId - b.lessonId || a.id - b.id);

        setItems(uniq);
      } catch {
        setError("Nie udało się pobrać załączników lekcji.");
        setItems([]);
      } finally {
        setLoading(false);
      }
  ...
  const handleDeactivate = async (id: number) => {
    try {
      await lessonAttachmentsApi.delete(id);
      setItems((prev) => prev.filter((x) => x.id !== id));
    } catch {
      setError("Nie udało się dezaktywować załącznika.");
    }
  };
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "LessonAttachments");
    ...
    const fileName = `tutor-lesson-attachments-${new Date()
      .toISOString()
      .slice(0, 10)}.xlsx`;
  ...
  return (
    <Box maxW="8xl" mx="auto" px={{ base: 4, md: 8 }} py={{ base: 8, md: 10 }}>
      <HStack justify="space-between" mb={2} spacing={3} flexWrap="wrap">
        <Heading>Załączniki lekcji (Tutor)</Heading>

        <Button
          colorScheme="yellow"
          variant="outline"
          onClick={() => navigate("/tutor/lesson-attachments/new")}
        >
          Utwórz
        </Button>
      </HStack>
      ...
      <Text color="gray.400" fontSize="md" mb={6}>
        Edytować i usuwać można tylko załączniki należące do lekcji zalogowanego
        tutora.
      </Text>
      ...
      <HStack mb={4} spacing={3} flexWrap="wrap" align="start">
        <Box flex="1" minW={{ base: "100%", md: "360px" }}>
          ...
          <Input
            ...
            placeholder="id, lekcja, nazwa, url"
            bg="gray.800"
                  ...
                  <Td textAlign="right">
                    <HStack justify="flex-end">
                      <Button
                        ...
                        variant="outline"
                        onClick={() =>
                          navigate(`/tutor/lesson-attachments/${x.id}`)
                        }
                      >
                        Edytuj
                      </Button>
