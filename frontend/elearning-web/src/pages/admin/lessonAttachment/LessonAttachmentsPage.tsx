...
export default function LessonAttachmentsAdminPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        setError(null);
        const res = await lessonAttachmentsApi.getAll();
        setItems(res.data ?? []);
      } catch {
  ...
  const handleExportExcel = () => {
    ...
    const fileName = `lesson-attachments-report-${new Date()
      .toISOString()
      .slice(0, 10)}.xlsx`;
  ...
  return (
    <Box maxW="8xl" mx="auto" px={{ base: 4, md: 8 }} py={{ base: 8, md: 10 }}>
      <HStack justify="space-between" mb={6} spacing={3} flexWrap="wrap">
        <Heading>Załączniki lekcji</Heading>

        <Button
          colorScheme="yellow"
          variant="outline"
          onClick={() => navigate("/admin/lesson-attachments/new")}
        >
          Utwórz
        </Button>
      </HStack>
      ...
      <HStack mb={4} spacing={3} flexWrap="wrap">
        <Input
          ...
          placeholder="Szukaj: id, lekcja, nazwa pliku"
          bg="gray.800"
                  ...
                  <Td textAlign="right">
                    <HStack justify="flex-end">
                      <Button
                        ...
                        variant="outline"
                        onClick={() =>
                          navigate(`/admin/lesson-attachments/${x.id}`)
                        }
                      >
                        Edytuj
                      </Button>
