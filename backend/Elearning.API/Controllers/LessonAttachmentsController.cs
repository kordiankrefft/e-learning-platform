...
{
    [ApiController]
    public class LessonAttachmentsController : BaseController
    {
        private readonly ILessonAttachmentService service;
        private readonly IUserCourseAccessService userCourseAccessService;

        public LessonAttachmentsController(ILessonAttachmentService service, IUserCourseAccessService userCourseAccessService, DatabaseContext databaseContext)
        ...
        [HttpPost]
        [Route(Urls.LESSON_ATTACHMENTS)]
        [Authorize(Roles = "Admin,Tutor")]
        public async Task<IActionResult> CreateAsync([FromBody] LessonAttachmentCreateDto dto)
        {
            ...
            if (isTutor && !isAdmin)
            {
                ...
                if (courseInfo == null)
                    return BadRequest("Nie znaleziono aktywnej lekcji powiązanej z tym załącznikiem.");
        ...
        [HttpPut]
        [Route(Urls.LESSON_ATTACHMENTS_ID)]
        [Authorize(Roles = "Admin,Tutor")]
        public async Task<IActionResult> EditAsync([FromBody] LessonAttachmentEditDto dto, int id)
        {
            if (dto.Id != id)
            ...
            if (isTutor && !isAdmin)
            {
                ...
                var courseInfo = await service.GetCourseInfoForAttachmentAsync(id);
                if (courseInfo == null)
                    return BadRequest("Nie znaleziono aktywnego załącznika o podanym id.");
        ...
        [HttpGet]
        [Route(Urls.LESSON_ATTACHMENTS_FOR_LESSON)]
        [Authorize(Roles = "Student,Tutor,Admin")]
